"""
Wrapper for aioboto3 WAF service.

[Documentation](https://youtype.github.io/types_aioboto3_docs/types_aiobotocore_waf/)

Copyright 2025 Vlad Emelianov

Usage::

    ```python
    from boto34.aioboto3 import Session

    # Wrapper for aioboto3.Session constructor
    # Returns boto34.Session inherited from aioboto3.Session
    session = Session()
    session: boto34.aioboto3.session.Session

    # Type annotated aioboto3.Client
    # Uses the same arguments as aioboto3.client method
    # Returns type annotated aioboto3 WAF client
    waf_client = session.waf.client()
    waf_client: types_aiobotocore_waf.client.WAFClient
    ```
"""

from __future__ import annotations

from boto34.aioboto3.service_factory import ServiceFactory

try:
    from types_aiobotocore_waf.client import WAFClient
except ImportError:
    WAFClient = object  # type: ignore[misc,assignment]


class WAFService(
    ServiceFactory[WAFClient]  # type: ignore[misc,assignment]
):
    SERVICE_NAME = "waf"
    _SERVICE_PROP = "waf"
