"""
Wrapper for boto3 ParallelComputingService service.

[Documentation](https://youtype.github.io/types_boto3_docs/types_boto3_pcs/)

Copyright 2025 Vlad Emelianov

Usage::

    ```python
    from boto34.boto3 import Session

    # Wrapper for boto3.Session constructor
    # Returns boto34.Session inherited from boto3.Session
    session = Session()
    session: boto34.boto3.session.Session

    # Type annotated boto3.Client
    # Uses the same arguments as boto3.client method
    # Returns type annotated boto3 ParallelComputingService client
    pcs_client = session.pcs.client()
    pcs_client: types_boto3_pcs.client.ParallelComputingServiceClient
    ```
"""

from __future__ import annotations

from types_boto3_pcs.client import ParallelComputingServiceClient

from boto34.boto3.service_factory import ServiceFactory


class ParallelComputingServiceService(ServiceFactory[ParallelComputingServiceClient]):
    SERVICE_NAME = "pcs"
    _SERVICE_PROP = "pcs"
