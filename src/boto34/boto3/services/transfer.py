"""
Wrapper for boto3 Transfer service.

[Documentation](https://youtype.github.io/types_boto3_docs/types_boto3_transfer/)

Copyright 2025 Vlad Emelianov

Usage::

    ```python
    from boto34.boto3 import Session

    # Wrapper for boto3.Session constructor
    # Returns boto34.Session inherited from boto3.Session
    session = Session()
    session: boto34.boto3.session.Session

    # Type annotated boto3.Client
    # Uses the same arguments as boto3.client method
    # Returns type annotated boto3 Transfer client
    transfer_client = session.transfer.client()
    transfer_client: types_boto3_transfer.client.TransferClient
    ```
"""

from __future__ import annotations

from boto34.boto3.service_factory import ServiceFactory

try:
    from types_boto3_transfer.client import TransferClient
except ImportError:
    TransferClient = object  # type: ignore[misc,assignment]


class TransferService(
    ServiceFactory[TransferClient]  # type: ignore[misc,assignment]
):
    SERVICE_NAME = "transfer"
    _SERVICE_PROP = "transfer"
